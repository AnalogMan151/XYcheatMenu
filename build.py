...
X_TID     = "0004000000055D00"
Y_TID     = "0004000000055E00"
FTP_FOLDER    = "plugin/"
PORT        = 5000
...
try:
    run("mkdir -p " + FTP_FOLDER + X_TID);
    run("mkdir -p " + FTP_FOLDER + Y_TID);
except Exception as e:
    pass
shutil.copy2(COPYTOPATH, "./" + FTP_FOLDER + X_TID + "/X.plg");
shutil.copy2(COPYTOPATH, "./" + FTP_FOLDER + Y_TID + "/Y.plg");
...
if (user == "yes" or user == "y"):
    ...
    connect(HOST, PORT);
    send(FTP_FOLDER + X_TID, "X.plg");
    send(FTP_FOLDER + Y_TID, "Y.plg");
    disconnect();
else:
